/*
 * Heart Rate Monitor - Arduino + Analog Pulse Sensor + SSD1306 OLED
 *
 * Reviewed portfolio version.
 * Changes relative to the original source:
 *   1) Configures ledPin as OUTPUT in setup().
 *   2) Corrects chronological traversal of the 5-sample circular buffer used
 *      to estimate BPM after the buffer wraps.
 *
...
int calcularBPM() {
  ...
  // indiceTempo aponta para a próxima posição de escrita.
  // Quando o buffer está cheio, essa posição também corresponde ao registro mais antigo.
  int quantidade = totalTempos;
  int indiceMaisAntigo = (totalTempos < 5) ? 0 : indiceTempo;

  for (int i = 0; i < quantidade - 1; i++) {
    int indiceAtual = (indiceMaisAntigo + i) % 5;
    int proximoIndice = (indiceMaisAntigo + i + 1) % 5;

    unsigned long intervalo = temposBatimentos[proximoIndice] - temposBatimentos[indiceAtual];
  ...
  if (intervalosValidos > 0) {
    ...
    int bpmCalculado = 60000UL / mediaIntervalo;
